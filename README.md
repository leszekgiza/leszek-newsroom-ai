...
- **AI Summaries** - 2-sentence intro + full summary (LLM, provider-agnostic)
- **Text-to-Speech** - Listen to summaries (TTS, provider-agnostic)
...
- **Frontend**: Next.js 16.1.1 (App Router), React 19, Tailwind CSS, Zustand
- **Backend**: Next.js API Routes, Prisma 7.2.x, PostgreSQL
- **Auth**: Custom JWT cookie auth (no NextAuth)
- **AI (LLM)**: Provider-agnostic (example: Anthropic Claude)
- **TTS**: Provider-agnostic (example: Edge TTS)
- **Scraping**: Crawl4AI (Python, Docker)

## Provider-agnostic + BYO Keys (Open Source)

Open Source users bring their own API keys for LLM/TTS. The project does not
include any free tiers or bundled usage. If no keys are provided, AI/TTS
features are disabled or use non-LLM fallbacks where possible.

Recommended env vars (examples only):
- `ANTHROPIC_API_KEY` (example LLM provider)
- `TTS_PROVIDER` / `TTS_API_KEY` (optional, if switching from Edge TTS)
